...
class HotStarMirrorProvider : MainAPI() {
        ...
        if (data.episodes.first() == null) {
            ...
            data.episodes.filterNotNull().mapTo(episodes) { ep ->
                newEpisode(LoadData(title, ep.id)) {
                    ...
                    this.posterUrl = "https://imgcdn.kim/hsepimg/150/${ep.id}.jpg"
    ...
    private suspend fun getEpisodes(
        ...
        while (true) {
            ...
            data.episodes?.mapTo(episodes) { ep ->
                newEpisode(LoadData(title, ep.id)) {
                    name = ep.t
                    episode = ep.ep.replace("E", "").toIntOrNull()
                    season = ep.s.replace("S", "").toIntOrNull()
                    this.posterUrl = "https://imgcdn.kim/hsepimg/${ep.id}.jpg"
                    this.runTime = ep.time.replace("m", "").toIntOrNull()
                }
            }
    ...
    override suspend fun loadLinks(
        ...
        return netMirrorMobileLoadLinks(
            ...
            baseCookies = mapOf("hd" to "on", "ott" to "hs"),
            name = name,
